...
void particleSys::drawMe(std::shared_ptr<Program> prog) {
  ...
  if (h_col != -1) glDisableVertexAttribArray(1);
}
...
void particleSys::update() {
  ...
  glm::decompose(theCamera, scale, rotation, translation, skew, perspective);
  sorter.C = glm::toMat4(rotation);
  ...
  //go through all the particles and update the CPU buffer
   for (int i = 0; i < numP; i++) {
        ...
        pointColors[i*4+0] =col.r + col.a/10; 
        pointColors[i*4+1] =col.g + col.a/10; 
        pointColors[i*4+2] =col.b + col.a/10;
        pointColors[i*4+3] =col.a;
